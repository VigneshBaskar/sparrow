          ...
          'concepts_list' : [
              {
                  ...
                  'class_data_dir': f"/home/ec2-user/finetuner/data/regularization_images/{gender}" # Path of regularization images
              }
          ],
    ...
    for concept in config['concepts_list']:
        ...
        if num_current_class_images < config['num_class_images']:
            ...
            if pipeline is None:
                ...
                sample_dataset = PromptDataset(concept['class_prompt'], config['num_class_images']-num_current_class_images)
...
def save_weights(step):
    if accelerator.is_main_process:
        ...
        if config['save_sample_prompt'] is not None:
            ...
            g_cuda = torch.Generator(device=accelerator.device).manual_seed(config['seed'] if config['seed'] else torch.randint(0, 1024, (1, 1)).item())
            pipeline.set_progress_bar_config(disable=True)
            with torch.autocast("cuda"), torch.inference_mode():
